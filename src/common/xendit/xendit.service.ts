...
export class XenditService {
  ...
  async resolveRequestRecurringPlan(
    ...
    totalAmount: number,
    user: User,
  ): Promise<CreateRecurringPlan> {
    const { plan, id } = newSubscription;
    const reference_id = `${plan.name}-${id}-${Date.now()}`;
    const customer_id = await this.getOrCreateCustomer(user).then(
    ...
    return {
      ...
      schedule: {
        ...schedule,
        reference_id: `${plan.name}-${schedule.interval}-${randomUUID()}-${Date.now()}`,
  ...
  async createRecurringPlan(
    ...
    return await firstValueFrom(
      this.xenHttp.post<XenditRecurringPlanData>('/recurring/plans', body).pipe(
        map((response) => response.data),
        catchError((error) => {
          const responseData = error.response?.data;

          this.logger.error(
            `Failed to create recurring plan: ${JSON.stringify(responseData, null, 2)}`,
          );
