...
import { ThrottlePolicy } from '../../../common/interfaces/throttle.interface';
import LoggerService from 'src/infrastructure/logger/logger.service';
import { Request, Response } from 'express';

import { InjectRedisClient } from '@quazex/nestjs-ioredis';
import { IpService } from '../request/ip.service';
import { ClientIdentityService } from '../request/client-identity.service';
...
export class ThrottleService {
  ...
  createThrottleKey(req: Request, policy: ThrottlePolicy) {
    ...
    const ip = this.ipService.getClientIp(req);
    ...
    const { identityHash } = this.clientIdentityService.generateHashes(
      unique,
      ip,
      {
        browser,
