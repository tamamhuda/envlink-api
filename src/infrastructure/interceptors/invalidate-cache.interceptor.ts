...
} from '../cache/decorators/invalidate-cache.decorator';
import { CacheKeyContext } from '../../common/interfaces/cache-context.interface';
  ...
  async intercept<T>(
    ...
    const baseCtx: CacheKeyContext<T> = {
      ...
      sessionId: authorization
        ? await this.jwtService.extractSessionIdFromHeader(request)
        : undefined,
    };
